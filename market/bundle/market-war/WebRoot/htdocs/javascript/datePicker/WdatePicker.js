 ...
 */
//在时间控件input表单里禁用退格,防止IE后退页面
$("html").on("keydown", "[onclick^=WdatePicker]", function (event) {
	if (event.which == 8) {
		return false
	}
});
...
(function () {
	...
	function U(J, C) {
		jQuery("iframe[src*=datePicker]", top.document).show();
		$dp.win = X;
		B();
		J = J || {};
		if (C) {
	...
	function K(H, $) {
		...
		if (!$dp.dd || $dp.eCont || ($dp.lang && $dp.realLang && $dp.realLang.name != $dp.lang && $dp.getLangIndex && $dp.getLangIndex($dp.lang) >= 0)) {
			...
			if (_.$dpPath == "") F(A);
			var B = "<iframe style=\"width:1px;height:1px\" src=\"" + ""+ 'htdocs/javascript/datePicker/' + "My97DatePicker.htm\" frameborder=\"0\" border=\"0\" scrolling=\"no\"></iframe>";
			if ($dp.eCont) {
